...
String dweet::getDweet(char* thing_name, char* key){
  ...
  if (_client.connect(SERVER, PORT)){
    ...
    while (!_client.available() && millis()-m<10000);
	if (millis()-m<10000) {
...
String dweet::getTimeDweet(char* thing_name){
  String value;
  String response;
  uint8_t bodyPosinit;
  uint8_t bodyPosend;
  ...
  if (_client.connect(SERVER, PORT)){
	...
	if (millis()-m<10000) {
		...
		response = response.substring(bodyPosinit);
		Serial.println(response);
		String date = response.substring(response.indexOf("created")+10,response.indexOf("created")+29);
